...
try:
    ...
    print(json.dumps({"error": "httpx is not installed. Please run: pip install httpx"}))
    sys.exit(1)
...
def get_citations(handle):
    ...
    try:
        ...
        if root.tag == "citationData":
            ...
            print(json.dumps({"error": "Unrecognized XML structure", "raw": response.text[:200]}))
            
    except ET.ParseError:
        print(json.dumps({"error": "Failed to parse XML response", "raw": response.text[:200]}))

def main():
    parser = argparse.ArgumentParser(description="Get citation counts for a RePEc Handle using CitEc API")
