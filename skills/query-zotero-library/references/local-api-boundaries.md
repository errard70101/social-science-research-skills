...
| Child attachments | `/api/users/0/items/{key}/children` |
...
collection. A bare collection name is accepted only when it is unique.
...
- Empty extracted pages: the PDF may be image-only and require OCR.
